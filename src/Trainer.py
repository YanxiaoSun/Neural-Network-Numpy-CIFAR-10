...
class Trainer:
    def __init__(self, model, optimizer, loss, n_epochs, eval_step):
        self.model = model
    ...
    def train(self, X_train, y_train, X_val, y_val, batch_size=256, save_ckpt=True, verbose=True):
        for epoch in range(1, self.n_epochs + 1):
            ...
            if epoch % self.eval_step == 0:
                valid_loss, valid_acc = self.evaluate(X_val, y_val)
        ...
        'train_acc_history': self.train_acc,
        'val_loss_history': self.valid_loss,
        'val_acc_history': self.valid_acc,
        }
    ...
    def evaluate(self, X, y, batch_size=256):
        ...
        for i in range(0, len(X), batch_size):
            ...
            loss = self.loss.forward(y_pred, y_batch)
            total_loss += loss * len(x_batch)
        valid_loss = total_loss / len(X)
        valid_acc = total_acc / len(X)
